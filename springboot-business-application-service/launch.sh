...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "springboot-business-application-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name springboot-business-application-service apps/springboot-business-application-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app springboot-business-application-service:1.0-SNAPSHOT
    oc expose svc/springboot-business-application-service
else

	echo "Launching the application locally..."
	pattern="springboot-business-application-service"
	files=( $pattern )
	cd ${files[0]}
