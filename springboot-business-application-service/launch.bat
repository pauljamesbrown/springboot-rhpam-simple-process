...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "springboot-business-application-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name springboot-business-application-service apps/springboot-business-application-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app springboot-business-application-service:1.0-SNAPSHOT
		call oc expose svc/springboot-business-application-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd springboot-business-application-service
		cd target
